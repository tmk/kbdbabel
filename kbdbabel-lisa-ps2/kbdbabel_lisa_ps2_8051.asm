; ---------------------------------------------------------------------
; Apple Lisa to AT/PS2 keyboard transcoder for 8051 type processors
;
; $KbdBabel: kbdbabel_lisa_ps2_8051.asm,v 1.2 2009/01/15 22:59:01 akurz Exp $
;
; Clock/Crystal: 24MHz.
;
; Lisa Keyboard connect:
...
; $ asl -L kbdbabel_lisa_ps2_8051.asm -o kbdbabel_lisa_ps2_8051.p
; $ p2bin -l \$ff -r 0-\$7ff kbdbabel_lisa_ps2_8051
; write kbdbabel_lisa_ps2_8051.asl -L kbdbabel_lisa_ps2_8051.asm -o kbdbabel_lisa_ps2_8051.p
...
RXBitCount	equ	33h	; Lisa Scan bit counter
RXBitBuf	equ	34h	; Lisa Scan bits
...
RXCompleteF	bit	B22.0	; full and correct byte-received
RXActiveF	bit	B22.1	; Keyboard Clock transmit active
LisaStartbit0	bit	B22.2	; first Lisa Protocol Start Bit, must be 1
LisaStartbit1	bit	B22.3	; second Lisa Protocol Start Bit, must be 0
LisaDataBit3a	bit	B22.4	; duplicated data bit 3
...
;----------------------------------------------------------
; timer 1 int handler: trigger and scan the Lisa keyboard
; this is a 16-step-loop:
; step 0: init timer to 15.5us
;----------------------------------------------------------
HandleTF1:
	push	acc			; 2,2
	push	psw			; 2,4

; -- switch by bit-number
	mov	dptr,#LisaScanStepJT	; 2,6
	mov	a,RXBitCount		; 1,7
	add	a,acc			; 1,8
	anl	a,#1fh			; 1,9
	jmp	@a+dptr			; 2,11

LisaScanStepJT:
	sjmp	LisaScanTimerFast	; Step 0: set the timer to Lisa-scan-rate@ 15.5uS
	sjmp	HandleTF1Done		; Step 1: nop
	sjmp	LisaScanBit0		; Step 2: Start Init from Host
	sjmp	LisaScanBit1		; Step 3: Clear Init.
	sjmp	LisaScanStartBit0	; Step 4: get first Start-Bit
	sjmp	LisaScanStartBit1	; Step 5: get second Start-Bit
	sjmp	LisaScanDataBit		; Step 6-9: read data bits 0-3
	sjmp	LisaScanDataBit
	sjmp	LisaScanDataBit
	sjmp	LisaScanDataBit
	sjmp	LisaScanDataBit3a	; Step 10: check double data bit 3
	sjmp	LisaScanDataBit		; Step 11-14: read data bits 4-7
	sjmp	LisaScanDataBit
	sjmp	LisaScanDataBit
	sjmp	LisaScanDataBit
	sjmp	LisaScanShow		; Step 15: reset the timer to 10ms intervals, show results
	sjmp	LisaScanTimerSlow	; safety

; -- set timer to 10ms
LisaScanTimerSlow:
	acall	timer1_10ms_init
	clr	RXActiveF
	sjmp	HandleTF1Done
...
LisaScanShow:
	...
	clr	RXActiveF
	mov	a,RXBitBuf
	; blank scancode -> no data available
	cjne	a,#0ffh,LisaScanShowYes
	sjmp	HandleTF1Done
LisaScanShowYes:
	; first start bit must be 0
	jnb	LisaStartbit0,HandleTF1Done
	; second start bit must be 1
	jb	LisaStartbit1,HandleTF1Done
	mov	RawBuf,a
	setb	RXCompleteF
;	mov	p1,a
;	cpl	a
...
; -- data bits, this is time-critical
LisaScanDataBit:
	inc	RXBitCount		; 1
	mov	a,RXBitBuf		; 1
	mov	c,p3.2			; 1
...
;----------------------------------------------------------
;----------------------------------------------------------
; Lisa to AT translaton table
; note: obsolete Lisa-Scancode-bit3 skipped to conserve ROM-Space
;----------------------------------------------------------
Lisa2ATxlt0	DB	 11h,  31h,  44h,   0h,  0h,  5ah,  0h,  0h
Lisa2ATxlt1	DB	 12h,  32h,  49h,  14h,  0h,  7ah,  0h,  0h
Lisa2ATxlt2	DB	 58h,  21h,  41h,  69h,  0h,  72h,  0h,  0h
Lisa2ATxlt3	DB	 14h,  2ah,  29h,  4ah,  0h,  71h,  0h,  0h
Lisa2ATxlt4	DB	 23h,  33h,  52h,   0h,  0h,  71h,  0h,  0h
Lisa2ATxlt5	DB	 22h,  34h,  4ch,  00h,  0h,  74h,  0h,  0h
Lisa2ATxlt6	DB	 1ah,  2bh,  4bh,  70h,  0h,  73h,  0h,  0h
Lisa2ATxlt7	DB	 0dh,  76h,  3ah,  5ah,  0h,  6bh,  0h,  0h
Lisa2ATxlt8	DB	 1dh,  35h,  5bh,   0h,  0h,  4ah,  0h,  0h
Lisa2ATxlt9	DB	 1bh,  2ch,  54h,  11h,  0h,  7dh,  0h,  0h
Lisa2ATxlta	DB	 15h,  2dh,  42h,  66h,  0h,  75h,  0h,  0h
Lisa2ATxltb	DB	 16h,  2eh,  3bh,  4dh,  0h,  6ch,  0h,  0h
Lisa2ATxltc	DB	 25h,  3eh,  43h,  61h,  0h,  7ch,  0h,  0h
Lisa2ATxltd	DB	 26h,  3dh,  3ch,  5dh,  0h,  79h,  0h,  0h
Lisa2ATxlte	DB	 1eh,  36h,  45h,  55h,  0h,  7bh,  0h,  0h
Lisa2ATxltf	DB	 1ch,  24h,  46h,  4eh,  0h,  77h,  0h,  0h
...
;----------------------------------------------------------
Lisa2ATxlte0	DB	 00h,  00h,  00h,  00h,  00h,  01h,  00h,  00h
Lisa2ATxlte1	DB	 00h,  00h,  00h,  01h,  00h,  00h,  00h,  00h
Lisa2ATxlte2	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlte3	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlte4	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlte5	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlte6	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlte7	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlte8	DB	 00h,  00h,  00h,  00h,  00h,  01h,  00h,  00h
Lisa2ATxlte9	DB	 00h,  00h,  00h,  01h,  00h,  00h,  00h,  00h
Lisa2ATxltea	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlteb	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxltec	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxlted	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxltee	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
Lisa2ATxltef	DB	 00h,  00h,  00h,  00h,  00h,  00h,  00h,  00h
...
;----------------------------------------------------------
TranslateToBuf:
	; get Scancode-bit 3 / make/break-bit
	mov	a,RawBuf
	mov	c,acc.3
	mov	ATTXBreakF,c

	; eleminate bit3, for compressed translation table
	mov	c,acc.4
	mov	acc.3,c
	mov	c,acc.5
	mov	acc.4,c
...
; 15.5mus intervals
; 15.5mus@24.000MHz -> th0 and tl0=225 ; (256-24000*0.0155/12)
;----------------------------------------------------------
timer1_init:
	clr	tr1
	anl	tmod, #0fh	; clear all upper bits
	orl	tmod, #20h	; M0,M1, bit4,5 in TMOD, timer 1 in mode 1, 16bit
	mov	th1, #225
	mov	tl1, #225
...
; Id
;----------------------------------------------------------
RCSId	DB	"$KbdBabel: kbdbabel_lisa_ps2_8051.asm,v 1.2 2009/01/15 22:59:01 akurz Exp $"
